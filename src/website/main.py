...
from website.shops import group_17
...
app.include_router(group_17.router, prefix="/api/17", tags=["17 Shop"])
...
async def shop_web(request: Request, shop_id: str, db: Session = Depends(get_db), user: DBUser = Depends(get_current_user)):
    ...
    shop_module = {"1": group_1,"17": group_17}.get(shop_id)
...
async def results_page(request: Request, db: Session = Depends(get_db)):
    ...
    shop_modules = {"1": group_1,"17": group_17}
